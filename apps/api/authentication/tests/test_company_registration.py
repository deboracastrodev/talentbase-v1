...
class TestCompanyRegistrationView:
    ...
    def test_register_company_duplicate_email(self):
        ...
        User.objects.create_user(
            email="duplicate@example.com", password="ExistingPass123!", role="candidate"
        )
    ...
    def test_register_company_sends_two_emails(self, mock_send_email):
        ...
        # Story 2.7: Only company notification email is sent (AC7)
        # Admin notification (AC8) will be implemented in future story
        assert mock_send_email.call_count == 1

        # Verify company notification email (AC7)
        call_kwargs = mock_send_email.call_args[1]
        assert call_kwargs["template_name"] == "company_registration_submitted"
        assert call_kwargs["recipient_email"] == "company@example.com"
        assert "Aguardando" in call_kwargs["subject"]
